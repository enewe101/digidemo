...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
		for vote in orm.ProposalVote.objects.all():
			vote.target = vote.proposal
			vote.save()

		for vote in orm.LetterVote.objects.all():
			vote.target = vote.letter
			vote.save()

    def backwards(self, orm):
		for vote in orm.ProposalVote.objects.all():
			vote.proposal = vote.target
			vote.save()

		for vote in orm.LetterVote.objects.all():
			vote.letter = vote.target
			vote.save()
